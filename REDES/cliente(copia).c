...
int main ( )
{
   	...
   	printf ("Envio %d\n", Datos);
		...
		-----------------------------------------------------------------------*/
		int enviado = sendto (Socket_Cliente, (char *) &Datos, sizeof(Datos), 0, // como Datos se ha definido como entero lo que hacemos es un casting para que quede como char*
   	...
   	if (enviado < 0)
		...
		/*----------------------------------------------------------------------
			...
			----------------------------------------------------------------------- */ 
			int recibido = recvfrom (Socket_Cliente, (char *)&Datos, sizeof(Datos), 0, // si el servidor no esta activo nos vamos a quedar bloqueado
			(struct sockaddr *) &Servidor, &Longitud_Servidor);
