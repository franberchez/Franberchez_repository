...
SIGINT Sse genera cuando el usuario pulsa Ctrl+C, al pulsarlo nuestro programa nos cerrara las colas servidora y cliente,
lo cual veremos reflejado porque se nos cerraran los terminales de ambos ejecutables

*/

void mi_manejador_sigint(int signal)
{
    printf("Capturé la señal SIGINT y salgo!\n");
    finPrograma(-1);
}
...
int main(int argc, char **argv)
{
	...
	// Compruebo si he recibido alguna señal
	...
	do 
	{
		...
		if(mq_send(mq_server, buffer, MAX_SIZE, 0) != 0) //voy a enviar por la cola servidora lo que acabo de leer
		{
	...
	}while (!must_stop); // Iterar hasta escribir el código de salida
...
Nuestro manejador de llamadas SIGKILL
