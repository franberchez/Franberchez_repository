...
int main(int argc, char **argv)
{
	...
	int eflag = 0; // Podemos omitir esta variable si lo necesitamos
	...
	// Inicializar los atributos de la cola que solo hayq ue incializarlos cuando abrimos la cola con O_CREAT
	attr.mq_maxmsg = 10;        // Maximo número de mensajes
	attr.mq_msgsize = MAX_SIZE; // Maximo tamaño de un mensaje

	// Nombre para las colas
	sprintf(clientQueue, "%s-%s", CLIENT_QUEUE, getenv("USER")); // cojo mi usuario para que no haya problemas en el sistema
	sprintf(serverQueue, "%s-%s", SERVER_QUEUE, getenv("USER"));
	
	opterr = 0; // Esto en un principio si no queremos ponerlo tambien podemos obviarlo
	...
	if( mq_client == (mqd_t)-1)
	{
    	...
    	funcionLog("Error al abrir la cola cliente");
   		exit(EXIT_FAILURE);
	}
	...
	do {
		...
		// Comprobar el fin del bucle, aqui es donde comparo si recibo un exit, en caso de que lo haya recibido pongo a 1 mi bandera de salida
		...
		else
		{
			...
			else
			{
				...
				funcionLog("Fallo el matching de la expresion regular");
