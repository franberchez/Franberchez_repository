...
// #include <opencv2/xfeatures2d.hpp> I comment that library because it couse me some trouble on my computer
...
int compute_lbp_from_list(const std::vector<std::string> & lfiles, cv::Mat & train_lbp, const int * ncells, const bool normalize, const bool asrows, const bool withmirror)
{
  ...
  for (int fix =0; fix < lfiles.size(); fix++)
  {
    ...
    image = imread(imagePath, CV_8UC1);
    fsiv_lbp_desc(image, lbp_desc, ncells, normalize);
    train_lbp.push_back(lbp_desc);
	}
