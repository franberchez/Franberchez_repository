...
// ./test_detector test_detector_dataset.txt -model=inter_model -stride=10 -thrdet=1 -l=3
...
main(int argc, char * argv[])
{
    ...
    string modelname = parser.get<string>("model");
     ...
     int ncells[] = {10,8}; // Rows x cols // 6x4
     //
    ...
    while(dataset && !dataset.eof())
    {
        ...
        dnn::NMSBoxes(lRs, lscores, iou_th, nmsThreshold, indices);
        ...
        for (int i = 0; i < annotated_boxes.size(); i++)
        {
            ...
            rectangle(image_orig, annotated_boxes[i], Scalar(0,255,0),3,8,0);
				//
        }
        ...
        for (int i = 0; i < detections.size(); i++)
        {
            ...
            rectangle(image_orig, detections[i], Scalar(rand()%256,0,rand()%256),3,8,0);
            //imshow("Image", image);
            //imshow("Recatangle", image);
            //waitKey(0);
